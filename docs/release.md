...
`@agent-dispatch/store-sqlite` is published after `@agent-dispatch/core`.

## Prerequisites

- Publish `@agent-dispatch/core` for the target compatibility line.
- Configure npm Trusted Publisher for `agent-dispatch/store-sqlite` using workflow `.github/workflows/publish.yml`.
